...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (raw.includes('api.binance.com')) {
    ...
    for (const host of binanceHosts) {
      try { u.host = host; const r = await nativeFetch(u, {...init, signal:AbortSignal.timeout(2500)}); if (r.ok) return r; last = new Error(`Binance HTTP ${r.status}`); } catch (e) { last = e; }
    }
