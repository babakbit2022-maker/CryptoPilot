...
import { installAdminRoutes } from './admin-module.js';
...
const coinbaseSymbols = {BTC:'BTC',ETH:'ETH',SOL:'SOL',BNB:'BNB',XRP:'XRP',DOGE:'DOGE',ADA:'ADA',AVAX:'AVAX',LINK:'LINK',DOT:'DOT',LTC:'LTC',TRX:'TRX'};
...
function coinbaseProductFromBinance(pair) {
  const symbol = String(pair).replace(/USDT$/,'').toUpperCase();
  const base = coinbaseSymbols[symbol];
  if (!base) throw new Error('No Coinbase pair');
  return `${base}-USD`;
}

function coinbaseGranularity(interval) {
  return ({'15m':900,'1h':3600,'4h':14400,'1d':86400})[interval] || 900;
}

async function coinbaseKlinesFromBinanceRequest(url) {
  const u = new URL(url);
  const pair = u.searchParams.get('symbol');
  const interval = u.searchParams.get('interval') || '15m';
  const limit = Math.min(300, Math.max(50, Number(u.searchParams.get('limit') || 250)));
  const product = coinbaseProductFromBinance(pair);
  const granularity = coinbaseGranularity(interval);
  const end = Math.floor(Date.now()/1000);
  const start = end - granularity * limit;
  const r = await nativeFetch(`https://api.exchange.coinbase.com/products/${product}/candles?granularity=${granularity}&start=${start}&end=${end}`, {headers:{accept:'application/json'}, signal:AbortSignal.timeout(9000)});
  if (!r.ok) throw new Error(`Coinbase candles HTTP ${r.status}`);
  const rows = await r.json();
  if (!Array.isArray(rows) || !rows.length) throw new Error('Coinbase candles empty');
  const sorted = rows.sort((a,b)=>Number(a[0])-Number(b[0])).slice(-limit);
  const out = sorted.map(x=>{const t=Number(x[0])*1000,low=Number(x[1]),high=Number(x[2]),open=Number(x[3]),close=Number(x[4]),volume=Number(x[5]);return [t,open,high,low,close,volume,t+granularity*1000-1,close*volume,0,volume,0,0];});
  return new Response(JSON.stringify(out),{status:200,headers:{'content-type':'application/json','x-cryptopilot-provider':'coinbase-fallback'}});
}
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (raw.includes('api.binance.com')) {
    ...
    for (const host of binanceHosts) {
      try { u.host = host; const r = await nativeFetch(u,{...init,signal:AbortSignal.timeout(2500)}); if(r.ok)return r; last=new Error(`Binance HTTP ${r.status}`); }
      catch(e){ last=e; }
    }
    try { return await coinbaseKlinesFromBinanceRequest(raw); } catch(e) { last=e; }
    throw last || new Error('Market providers unavailable');
  }
...
// Express does not provide res.cookie/res.clearCookie by itself. Add a small native implementation so auth works.
express.application.response.cookie = function(name,value,options={}) {
  ...
  if(options.sameSite) parts.push(`SameSite=${options.sameSite==='lax'?'Lax':options.sameSite==='strict'?'Strict':'None'}`);
  this.append('Set-Cookie',parts.join('; '));
  return this;
};
express.application.response.clearCookie = function(name,options={}) { return this.cookie(name,'',{...options,maxAge:0}); };
