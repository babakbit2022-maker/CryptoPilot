...
app.post('/api/translate',aiLimit,async(req,res)=>{try{const language=String(req.body?.language||'en').toLowerCase();const texts=Array.isArray(req.body?.texts)?req.body.texts.map(x=>String(x)).filter(Boolean).slice(0,140):[];if(!['en','fa','ru'].includes(language)||!texts.length)return res.json({translations:{}});if(!process.env.OPENAI_API_KEY)return res.json({translations:Object.fromEntries(texts.map(t=>[t,t]))});const prompt='Translate each UI string into '+(language==='fa'?'Persian (Farsi)':language==='ru'?'Russian':'English')+'. Return ONLY a JSON object mapping each original string to its translation. Preserve numbers, symbols, crypto tickers, placeholders, HTML-free punctuation, and meaning. Strings: '+JSON.stringify(texts);const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+process.env.OPENAI_API_KEY},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-5.6-luna',input:prompt}),signal:AbortSignal.timeout(20000)});if(!r.ok)throw new Error('translate');const j=await r.json();let out={};try{out=JSON.parse(j.output_text||'{}')}catch{};res.json({translations:out});}catch{res.json({translations:{}});}});
...
async function geckoOHLC(symbol,tf,limit=250){const ids={BTC:'bitcoin',ETH:'ethereum',SOL:'solana',BNB:'binancecoin',XRP:'ripple',DOGE:'dogecoin',ADA:'cardano',AVAX:'avalanche-2',LINK:'chainlink',DOT:'polkadot',LTC:'litecoin',TRX:'tron'};const id=ids[symbol];if(!id)throw new Error('gecko_ohlc');const days=tf==='1d'?90:tf==='4h'?30:tf==='1h'?14:1;const u=`https://api.coingecko.com/api/v3/coins/${encodeURIComponent(id)}/ohlc?vs_currency=usd&days=${days}`;const r=await fetch(u,{headers:{accept:'application/json'},signal:AbortSignal.timeout(10000)});if(!r.ok)throw new Error('gecko_ohlc');const data=await r.json();if(!Array.isArray(data)||data.length<10)throw new Error('gecko_ohlc');return data.slice(-limit).map(x=>[Number(x[0]),Number(x[1]),Number(x[2]),Number(x[3]),Number(x[4]),0]);}
async function getAnalysis(pair,tf){
  ...
  try{
    const raw=await geckoOHLC(symbols[pair],tf,250);
    const rows=raw.map(x=>({t:+x[0],o:+x[1],h:+x[2],l:+x[3],c:+x[4],v:+x[5]}));
    const v={symbol:symbols[pair],pair,tf,provider:'coingecko_ohlc_fallback',updatedAt:new Date().toISOString(),candles:rows.map(x=>[x.t,x.o,x.h,x.l,x.c,x.v]),analysis:analyze(rows)};
    cache.set(key,{t:now,v});
    return v;
  }catch{}
...
app.get('/api/scanner',async(req,res)=>{const tf=String(req.query.tf||'15m');if(!tfMap[tf])return res.status(400).json({error:'unsupported_tf'});const key='__scanner_'+tf,now=Date.now(),cached=cache.get(key);if(cached&&now-cached.t<20000)return res.json(cached.v);await refreshMarketUniverse();const results=[];await Promise.all(Object.keys(symbols).slice(0,100).map(async pair=>{try{const j=await getAnalysis(pair,tf),a=j.analysis;results.push({symbol:j.symbol,pair,price:a.price,bullScore:a.bullScore,bearScore:a.bearScore,riskScore:a.riskScore,setup:a.setup,rsi:a.rsi,momentum:a.momentum,volumeRatio:a.volumeRatio,provider:j.provider});}catch{}}));results.sort((a,b)=>Math.max(b.bullScore,b.bearScore)-Math.max(a.bullScore,a.bearScore));const v={tf,updatedAt:new Date().toISOString(),results,count:results.length};cache.set(key,{t:now,v});res.json(v);});app.get('/robots.txt',(req,res)=>{const base=process.env.PUBLIC_BASE_URL||`${req.protocol}://${req.get('host')}`;res.type('text/plain').send(`User-agent: *\nAllow: /\nSitemap: ${base}/sitemap.xml\n`);});
