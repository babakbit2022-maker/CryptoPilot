...
try { db.exec("ALTER TABLE users ADD COLUMN premium_until TEXT"); } catch {}
db.exec(`
CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT UNIQUE NOT NULL,password TEXT NOT NULL,plan TEXT NOT NULL DEFAULT 'free',role TEXT NOT NULL DEFAULT 'user',email_verified INTEGER NOT NULL DEFAULT 0,premium_until TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
app.get('/api/account',auth,(req,res)=>{const u=db.prepare('SELECT id,email,plan,role,email_verified,premium_until,created_at FROM users WHERE id=?').get(req.user.id);res.json({ok:true,user:{...u,...accountState(u)}});});
